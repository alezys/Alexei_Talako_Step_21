...
function changeWindow(event) {
	if (event.target.id === squareBlock.id) {
		checkDrop = 1;
	}
}
function changeWindowOff(event) {
	if (event.target.id === squareBlock.id) {
		checkDrop = 0;
	}
}
function onMouseMove(event) {
	if (checkDrop === 1) {
		innerBlock.style.setProperty(`--widthBlock`, `${event.pageX}px`);
		innerBlock.style.setProperty(`--heightBlock`, `${event.pageY - 830}px`);
		}
	}
squareBlock.addEventListener(`mousedown`, changeWindow);
squareBlock.addEventListener(`mouseup`, changeWindowOff);
